...
function GameSelection() {
  ...
  const [isCreatingSP, setIsCreatingSP] = useState(false); // For single player button

  // --- Validation ---
  const validateForm = (isSinglePlayer = false) => {
    const newErrors = {};
  ...
  const handleCreateMultiplayerGame = async () => {
    if (!validateForm(false)) { // Pass false for isSinglePlayer
        toast.warn("Please fix the errors in the form for multiplayer game.");
        return;
    }
    ...
    try {
        ...
        toast.error(`Multiplayer Game Creation Failed: ${errorMessage}`);
  ...
  const handleStartSinglePlayerGame = () => {
    if (!validateForm(true)) { // Pass true for isSinglePlayer
        toast.warn("Please fix the errors in the form for single player game.");
        return;
    }
    setIsCreatingSP(true);
    ...
    const gameConfig = {
        ...
        timePerQuestion: Math.min(60, Math.max(2, timePerQuestion)),
        numLives: Math.min(10, Math.max(1, numLives)),
        cooldownSeconds: Math.min(10, Math.max(1, cooldownSeconds)), // Cooldown for showing results
        allowedQuestionTypes: selectedQuestionTypes,
        // gameMode is not applicable for single player
    };

    // Simulate a short delay if needed, then navigate
    // For now, navigate directly
    navigate('/quiz/single-player', { state: { gameConfig } });
    // setIsCreatingSP(false); // Navigation will unmount, so not strictly needed
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-gray-800 rounded-lg shadow-xl text-center">
      ...
      <div className="mb-8 p-4 bg-gray-700 rounded-md shadow">
        ...
        {/* Host Name */}
         <div className="mb-4 flex flex-col items-start">
            ...
            <input
              ...
              disabled={isLoading || isCreatingSP} aria-invalid={!!errors.hostName} aria-describedby="host-name-error"
            />
        ...
        <div className="mb-4">
          ...
          <div className={`flex flex-wrap gap-3 justify-center p-2 rounded border ${errors.categories ? 'border-danger' : 'border-transparent'}`}>
            {CATEGORIES.map((category) => {
              ...
              return (
                <label key={category.id} className={`flex items-center space-x-2 px-3 py-1.5 rounded-md cursor-pointer transition duration-200 border ${ isSelected ? 'bg-primary-dark text-white border-primary-light ring-1 ring-primary-light' : 'bg-gray-600 hover:bg-gray-500 text-textPrimary border-gray-600 hover:border-gray-500' } ${isLoading || isCreatingSP ? 'opacity-50 cursor-not-allowed' : ''}`}>
                  <input type="checkbox" id={`category-${category.id}`} value={category.id} checked={isSelected} onChange={handleCategoryChange} className="form-checkbox h-4 w-4 text-primary rounded border-gray-400 focus:ring-primary-light disabled:opacity-50" disabled={isLoading || isCreatingSP} />
            ...
            {QUESTION_TYPES.map((qType) => {
              ...
              return (
                <label key={qType.id} className={`flex items-center space-x-2 px-3 py-1.5 rounded-md cursor-pointer transition duration-200 border ${ isSelected ? 'bg-indigo-700 text-white border-indigo-500 ring-1 ring-indigo-500' : 'bg-gray-600 hover:bg-gray-500 text-textPrimary border-gray-600 hover:border-gray-500' } ${isLoading || isCreatingSP ? 'opacity-50 cursor-not-allowed' : ''}`}>
                  <input type="checkbox" id={`qtype-${qType.id}`} value={qType.id} checked={isSelected} onChange={handleQuestionTypeChange} className="form-checkbox h-4 w-4 text-indigo-600 rounded border-gray-400 focus:ring-indigo-500 disabled:opacity-50" disabled={isLoading || isCreatingSP} />
        ...
        <div className="mb-4">
            ...
            <div className={`space-y-2 text-left p-2 rounded border ${errors.gameMode ? 'border-danger' : 'border-transparent'}`}>
                {GAME_MODES.map((mode) => {
                    ...
                    return (
                        <label key={mode.id} className={`flex items-start p-3 rounded-md bg-gray-600 hover:bg-gray-500 cursor-pointer border-2 transition-colors duration-200 ${ isSelected ? 'border-primary ring-1 ring-primary-light bg-primary-dark' : 'border-transparent' } ${isLoading || isCreatingSP ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            <input type="radio" name="gameMode" value={mode.id} checked={isSelected} onChange={handleGameModeChange} className="form-radio h-5 w-5 text-primary mt-0.5 mr-3 focus:ring-primary-light border-gray-400 disabled:opacity-50" disabled={isLoading || isCreatingSP} />
                            <div>
        ...
        <button onClick={handleCreateMultiplayerGame} disabled={isLoading || isCreatingSP} className={`w-full md:w-auto bg-success hover:bg-success-dark text-white font-bold py-3 px-8 rounded-lg text-lg transition duration-200 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-success focus:ring-opacity-50 shadow-lg flex items-center justify-center ${isLoading || isCreatingSP ? 'opacity-50 cursor-wait' : ''}`}>
          {isLoading ? ( <><Spinner size="sm" /><span className="ml-2">Creating Multiplayer...</span></> ) : ( 'Create Multiplayer Game' )}
        </button>
      </div>

      {/* Single Player Game Section */}
      <div className="mb-8 p-4 bg-gray-700 rounded-md shadow">
        <h2 className="text-2xl font-semibold mb-4 text-info-light border-b border-gray-600 pb-2">Play Solo</h2>
        <p className="text-sm text-textSecondary mb-4">
            Test your knowledge in a single-player game. Settings like Categories, Question Types, Number of Questions, Time, and Lives above will be used. Host Name and Game Mode are for multiplayer only.
        </p>
        <button
            onClick={handleStartSinglePlayerGame}
            disabled={isLoading || isCreatingSP}
            className={`w-full md:w-auto bg-info hover:bg-info-dark text-white font-bold py-3 px-8 rounded-lg text-lg transition duration-200 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-info focus:ring-opacity-50 shadow-lg flex items-center justify-center ${isLoading || isCreatingSP ? 'opacity-50 cursor-wait' : ''}`}
        >
          {isCreatingSP ? ( <><Spinner size="sm" /><span className="ml-2">Starting Single Player...</span></> ) : ( 'Start Single Player Game' )}
        </button>
      </div>
      ...
      <div className="p-4 bg-gray-700 rounded-md shadow">
         ...
         <button onClick={handleJoinGame} disabled={isLoading || isCreatingSP} className={`w-full md:w-auto bg-warning hover:bg-warning-dark text-gray-900 font-bold py-3 px-8 rounded-lg text-lg transition duration-200 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-warning focus:ring-opacity-50 shadow-lg ${isLoading || isCreatingSP ? 'opacity-50 cursor-not-allowed' : ''}`}>
            Join Game
          </button>
      </div>
    </div>
